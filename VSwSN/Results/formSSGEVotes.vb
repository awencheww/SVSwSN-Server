...
#Region "GOVERNOR"
    ...
    Private Sub LoadGov()
        Try
            query = "SELECT CandidateID, CONCAT(Lastname, ', ', Firstname, ' ', Middlename) as CanName,PartyName,PicFile,Votes FROM Candidate as C, Party as P, Positions as Po, voting_level as L WHERE L.LevelID = Po.LevelID AND C.PartyID = P.PartyID AND C.PositionID = Po.PositionID AND PositionName = 'Governor' AND LevelName = 'SSG Executive Branch'  ORDER BY Lastname, Firstname ASC "
            Connection()
            sqlcommand = New MySqlCommand(query, conn)
            sqlreader = sqlcommand.ExecuteReader
